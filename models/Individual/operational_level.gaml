...
species avoiding_target parent: operational_behavior {
	...
	float preferred_speed <- gauss(1.0, 0.26) min: 0.25 max: 1.74;
...
species avoiding parent: operational_behavior {
	float reaction_time <- 0.54 #s;
	float preferred_speed <- gauss(1.34, 0.26) min: 0.5 max: 1.74;
	float A <- 1.0;
	float B <- 0.3;
...
species avoiding_SFM parent: operational_behavior {
	float reaction_time <- 0.54 #s;
//	float preferred_speed <- gauss(1.34, 0.26) min: 0.5 max: 1.74;
	//float preferred_speed <- gauss(0.8, 0.26) min: 0.25 max: 1.74;
...
//		point e <-  (i.local_target - i.location) / distance_to (i.local_target, i.location);
//		//point target_force <- (e * preferred_speed - i.velocity) / reaction_time;
//		point target_force <- (e * MAXIMUM_SPEED - i.velocity) / reaction_time;
//		
//		// compute repulsion force with other pedestrians
//		point pedestrian_forces;
//		//loop ped over: i.pedestrians { // warmsings about dead pedestrians
//		
//		if (i.current_mesh != nil) {
//			int k <- 4;
//			list<individual> ped_neighbors <- i.old_mesh = nil ? i.current_mesh.pedestrians : i.current_mesh.pedestrians + i.old_mesh.pedestrians;
//			ped_neighbors <- (length(ped_neighbors) >= k) ? ped_neighbors closest_to(self, k) : ped_neighbors;
//			ped_neighbors <- ped_neighbors - i;
//			
//			loop ped over: ped_neighbors {
//				if dead(ped) = false {
//					float d <- distance_to(i.location, ped.location);
//					point nij <- (i.location - ped.location) / d;
//					point force <- nij * A * exp((AGENT_RADIUS + AGENT_RADIUS - d) / B);
...
//		point total_force <- target_force + pedestrian_forces ;
//		
//		return  total_force / norm(total_force); 	// normalized value is not good for fundamental diagram
////		return  total_force;
//		
//	}
//}
