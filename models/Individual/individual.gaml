...
species individual {
	...
	bool is_in_dense_area3 (point p, high_density_zone zone) {
		if (p.x < zone.boundary[0]) {
			return false;
		}
		if (p.x > zone.boundary[1]) {
	...
	// Lyon simulation
	bool is_in_dense_area2 (point p, high_density_zone zone) {
		int x <- int( (location.x - zone.boundary[0]) / zone.CELL_SIZE_X );
		int y <- int( (location.y - zone.boundary[2]) / zone.CELL_SIZE_Y );
		
		if (x >= zone.boundary[0] and x <= zone.boundary[])  and (y >= 0 and y <= zone.GRID_WIDTH - 1) {
//			if [x, y] in [[0, 25], [0, 26], [0, 27], [0, 28], [0, 29], [1, 29], [2, 29],
//						  [1, 27], [1, 28], [2, 28]] {
//				return false;
			if [x, y] in zone.group_goals[1] and MODEL_NAME = 'multi_zone' {
				do die;
			}
			// exit condition
			if (- 0.92 * location.x + location.y > 50.56) and MODEL_NAME = 'FestivalofLightsexitmesh' {
				if is_at_dense_region  {
					target <- location.x < 18.84 ? {5, rnd(65.5, 69.5)} : {rnd(16.5, 21), 88};
				}
				return false;
...
//			// exit condition
//			if (- 0.92 * location.x + location.y > 50.56) and MODEL_NAME = 'all_CC' {
//				if is_at_dense_region  {
//					my_zone <- location.x < 18.84 ? high_density_zone(1) : high_density_zone(2);
//				}
//				
//				return false;
//			
...
//		if (p.x >= zone.boundary[0] and p.x <= zone.boundary[1]) and (p.y >= zone.boundary[2] and p.y <= zone.boundary[3]) {
//			// exit condition
//			if (- 0.92 * p.x + p.y > 50.56) and MODEL_NAME = 'FestivalofLightsexitmesh' {
//				if is_at_dense_region  {
//					target <- p.x < 18.84 ? {5, rnd(65.5, 69.5)} : {rnd(16.5, 21), 88};
//				}
//				return false;
//			} 
//			
//			if (- 0.92 * p.x + p.y > 50.56) and MODEL_NAME = 'all_CC' and zone = high_density_zone(0) {
//				if is_at_dense_region  {
//					my_zone <- p.x < 18.84 ? high_density_zone(1) : high_density_zone(2);
//				}
//				return true;
//			} 
//			
...
//				my_operational_behavior <- first(avoiding_target);
	...
	aspect default {
		draw circle(AGENT_RADIUS) color: color depth: d #m;
